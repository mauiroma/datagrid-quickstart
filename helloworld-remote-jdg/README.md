helloworld-remote-jdg: Example Using Simple Infinispan Cache and Servlets
=========================================
Author: Burr Sutter, Martin Gencur
Level: Intermediate
Technologies: Infinispan, CDI, Servlet, JSF
Summary: The `helloworld-remote-jdg` quickstart shows how to use Infinispan cache in clustered mode, with expiration enabled, and access it from Servlets or JSF pages.
Target Product: JDG
...
The `helloworld-remote-jdg` is a basic example that shows how to store and retrieve data to/from the cache. Users can access the cache
...
The `helloworld-remote-jdg` example works in _Library mode_. In this mode, the application and the data grid are running in the same
...
4. This will deploy `target/jboss-helloworld-remote-jdg.war` to the first running instance of the server.
...
6. This will deploy `target/jboss-helloworld-remote-jdg.war` to the second running instance of the server.
...
The application will be running at the following URLs:
   ...
   <http://localhost:8080/jboss-helloworld-remote-jdg>  (first server instance)
   <http://localhost:8180/jboss-helloworld-remote-jdg>  (second server instance)

You can test replication of entries in the following way:

1. Access first server at <http://localhost:8080/jboss-helloworld-remote-jdg> and insert key "foo" with value "bar"
2. Access second server at <http://localhost:8180/jboss-helloworld-remote-jdg> and do the following:
   * Click on "Get Some"
   * Get the value for key "foo"
   * Click "Put Some More"
   * Insert key "mykey" with value "myvalue"
3. Access the first server at <http://localhost:8080/jboss-helloworld-remote-jdg> and do the following:
   * Click on "Get Some"
...
<http://localhost:8080/jboss-helloworld-remote-jdg/TestServletPut>
<http://localhost:8180/jboss-helloworld-remote-jdg/TestServletGet>  (note the different port 8180)
