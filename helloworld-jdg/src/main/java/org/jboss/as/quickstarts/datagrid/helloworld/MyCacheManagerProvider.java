...
public class MyCacheManagerProvider {
    ...
    public DefaultCacheManager getCacheManager() {
        if (manager == null) {
            ...
            GlobalConfiguration glob = new GlobalConfigurationBuilder().clusteredDefault() // Builds a default clustered
                    // configuration
                    .transport().addProperty("configurationFile", "jgroups-tcp.xml") // provide a specific JGroups configuration
    ...
    public void luceneMetaDati() {
        ...
        cb.clustering()
            ...
            .idColumnName("KEY").idColumnType("VARCHAR2(255)")
            .dataColumnName("DATA").dataColumnType("BLOB")
            .timestampColumnName("VERSION").timestampColumnType("NUMBER(20)")
            .dataSource()
            .jndiUrl(datasourceProperties().getJndiName());
        
    }

    public void luceneLock() {
        ConfigurationBuilder cb = new ConfigurationBuilder();
        cb.clustering()
            .cacheMode(CacheMode.REPL_SYNC)
            .remoteTimeout(25000)
